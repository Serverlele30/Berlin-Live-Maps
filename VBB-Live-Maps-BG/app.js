...
async function loadVehicles() {
  ...
  try {
    const res = await fetch(`https://v6.vbb.transport.rest/radar?${bbox}&results=200&duration=30`);
...
function updateClock() {
  const now = new Date();
  ssTimeEl.textContent = now.toLocaleTimeString('de-DE', { hour12: false });
  ssDateEl.textContent = now.toLocaleDateString('de-DE', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: '2-digit'
